...
export default async function proxy(req: NextRequest) {
  ...
  // Handle maiganhua redirect to wheat-straw-art
  if (nextUrl.pathname.startsWith('/maiganhua')) {
    // Replace /maiganhua with /wheat-straw-art
    const newPathname = nextUrl.pathname.replace('/maiganhua', '/wheat-straw-art');
    const newUrl = new URL(`${newPathname}${nextUrl.search}${nextUrl.hash}`, nextUrl);
    console.log('<< middleware end, redirecting maiganhua to wheat-straw-art:', newUrl.toString());
    return NextResponse.redirect(newUrl, 301); // Permanent redirect
  }

  // do not use getSession() here, it will cause error related to edge runtime
  // const session = await getSession();
  // Temporarily disable session check due to EPERM error
